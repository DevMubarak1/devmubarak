...
export default function ProjectCard({ project, index }: ProjectCardProps) {
  return (
    <motion.div
      ...
      className="bg-white dark:bg-black rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden border border-slate-200 dark:border-gray-800"
    >
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.tech.map((tech, techIndex) => (
            <span
              key={techIndex}
              className="bg-slate-100 dark:bg-gray-800 text-slate-700 dark:text-gray-300 px-2 py-1 rounded text-xs"
            >
              {tech}
            </span>
          ))}
